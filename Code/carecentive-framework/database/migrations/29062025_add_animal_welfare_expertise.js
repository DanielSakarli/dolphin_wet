// Define the cities for which the tables should be created
const cities = ['animal_welfare_expertise'];

// Function to create tables for each city
exports.up = async function (knex) {
  for (const city of cities) {
    // Dolphins table
    await knex.schema.createTable(`${city}_dolphins`, function (table) {
    ...
    // Create good_feeding table for the city
    await knex.schema.createTable(`${city}_good_feeding`, function (table) {
      ...
      table.foreign('dolphin_id').references(`${city}_dolphins.dolphin_id`).onDelete('SET NULL').onUpdate('CASCADE');
      table.string('dolphin_name');
      table.tinyint('body_condition_score').unsigned();
      table.string('body_condition_score_comments');
      table.double('weight_measured');
      table.string('weight_measured_comments');
      table.tinyint('kcal_calculations').unsigned();
      table.string('kcal_calculations_comments');
      table.tinyint('blood_hydration').unsigned();
      table.string('blood_hydration_comments');
      table.tinyint('fish_quality').unsigned();
      table.string('fish_quality_comments');
      table.tinyint('fish_variety').unsigned();
      table.string('fish_variety_comments');
      table.tinyint('bwo_score').unsigned();
      table.double('bwo_3_months').unsigned();
      table.double('bwo_12_months').unsigned();
      table.string('file_path');
      table.timestamps(true, true);
    });
    
    
    // Create good_health table for the city
    await knex.schema.createTable(`${city}_good_health`, function (table) {
      ...
      table.foreign('dolphin_id').references(`${city}_dolphins.dolphin_id`).onDelete('SET NULL').onUpdate('CASCADE');
      table.string('dolphin_name');
      table.tinyint('normal_floatability').unsigned();
      table.string('normal_floatability_comments');
      table.tinyint('records_normal_floatability').unsigned();
      ...
      table.double('total_rake_marks_percentage').unsigned();
      table.double('old_rake_marks_percentage').unsigned();
      table.timestamps(true, true);
    });
    // Mental State table
    await knex.schema.createTable(`${city}_emotional_state`, function (table) {
    ...
    // good_behavior table
    await knex.schema.createTable(`${city}_appropriate_behaviour`, function (table) {
    ...
    // good_housing table
    await knex.schema.createTable(`${city}_good_housing`, function (table) {
...
exports.down = async function (knex) {
    for (const city of cities) {
      await knex.schema.dropTableIfExists(`${city}_good_feeding`);
      await knex.schema.dropTableIfExists(`${city}_good_health`);
      await knex.schema.dropTableIfExists(`${city}_emotional_state`);
      await knex.schema.dropTableIfExists(`${city}_appropriate_behaviour`);
      await knex.schema.dropTableIfExists(`${city}_good_housing`);
      await knex.schema.dropTableIfExists(`${city}_dolphins`);
    }
  };
